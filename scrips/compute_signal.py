...
やっていること（概要）:
  1. Alpha Vantage から USD/JPY の価格（5分足・15分足・1時間足）、
     米10年債利回り、WTI原油を取得する。
  2. 各時間足について「線形回帰チャネル」を計算し、直近の価格が
     チャネルのどこに位置するかで SELL / BUY / WAIT / GATE を判定する。
  3. 4つの時間足の判定を集計して、総合バイアス・信頼度・相場モードを決める。
  4. Entry / Take Profit / Stop Loss を、直近のチャネル（1時間足）から算出する。
  5. 結果を signal.json として書き出す（GitHub Actionsがコミットし、
...
設計方針:
  ...
  - 米10年債・WTIはAlpha Vantageで取れるため計算に使うが、
...
def fetch_fx_intraday(interval):
    """USD/JPYの分足データを取得し、[(timestamp, close), ...] を古い順で返す。"""
    data = http_get_json({
    ...
    series = data.get(key)
    if not series:
        raise RuntimeError(f"USD/JPY {interval} のデータが取得できませんでした: {data}")
    rows = sorted(series.items())  # timestamp昇順
    closes = [(ts, float(v["4. close"])) for ts, v in rows]
    return closes
...
def build_signal():
    ...
    m5 = fetch_fx_intraday("5min")
    time.sleep(13)  # 無料枠は 5リクエスト/分 のため間隔をあける
    m15 = fetch_fx_intraday("15min")
    time.sleep(13)
    h1 = fetch_fx_intraday("60min")
    time.sleep(13)
