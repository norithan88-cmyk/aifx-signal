...
# 回帰チャネル計算に使う直近バーの本数
# 2026-08-20、19ヶ月分の実データでLOOKBACK値の比較検証を実施。100(旧設定)は
# シグナル頻度が低すぎ(月平均約52件)、短くするほど件数・勝率・PFとも改善した。
# ただしREVERT_WINDOW(=10、1分足の反発判定に使う本数)に対してLOOKBACKが
# 小さくなりすぎると、回帰チャネルの基準線自体が判定対象の反発点に引っ張られる
# 自己参照的な状態になり、数値が不自然に良くなる(30を下回るあたりから顕著)。
# 30はこの問題が起きない下限として選定(件数2,071件・勝率78.7%・PF2.39、
# 100の頃の約2倍の頻度)。10は疑わしい領域だが、今後の実データでの検証用に
# generate_daily_analysis等とは別に、実験ログ(experiment_lookback.pyのTODO参照)で
# 並行して記録する。
LOOKBACK = 30
